...
from droidbot import device as Device
from droidbot import app as App
from droidbot.adapter import adb as ADB
...
def main():
	...
	for log in traffic_logs:						
		...
		for log_parameter in traffic_parameters:	
			...
			if len(requested_pii_set) == 0:
				continue
			# print(f'PIIs in request: {requested_pii_set}')		
			for key in requested_pii_set:
	...
	df.to_csv(f'{args.log_dir}/data_sharing_output/output.csv')
